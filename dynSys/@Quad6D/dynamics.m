function dx = dynamics(obj, ~, x, T1, T2, ~, dims)
...
function dx = dynamics_cell_helper(obj, x, T1, T2, dims, dim)
...
switch dim
  ...
  case 2
    dx = (-(1/obj.m)*obj.transDrag*x{dims==2})+...
          ((-1/obj.m)*sin(x{dims==5})*T1)+...
          ((-1/obj.m)*sin(x{dims==5})*T2);
  case 3
    dx = x{dims==4};
  case 4
    dx = (-1/obj.m)*(obj.m*obj.grav + obj.transDrag*x{dims==4}) +...
      ((1/obj.m)*cos(x{dims==5})*T1)+...
      ((1/obj.m)*cos(x{dims==5})*T2);
  case 5
    dx = x{dims==6};
  case 6
