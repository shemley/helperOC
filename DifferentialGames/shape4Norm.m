function data = shape4Norm(grid, ignoreDims, center, radius)
% Taken from Ian Mitchell's shapeCylinder. Documentation below
% Only changes are: data raised to 4th power instead of 2nd
%                   no 4th root taken before subtracting radius
% Scott Hemley 2/28/2018
%
% 
% %%% Begin Documentation from Ian Mitchell's shapeCylinder %%%
%
...
for i = 1 : grid.dim
  if(all(i ~= ignoreDims))
    data = data + (grid.xs{i} - center(i)).^4;
  end
end
data = data - radius;
