...
data0 = shape4Norm(g, [3 4], targetCenter, R); % target set for attacker is a circle, ignore defender position
% also try shapeRectangleByCorners, shapeSphere, etc.

% Add static obstacles for defender to target region
% Uncomment for static obstacles for defender
...
% input bounds
% control limits (attacker)
uMax = [2;2];
% do dStep1 here
% disturbance limits (defender)
dMax = [1;1]; % apply same control limit to defender
...
dynSys = KinVehicleND2Agent_IndepCtrl([0,0,0,0], uMax, dMax); %do dStep3 here
...
save ./DifferentialGames/IROS_ValFunc.mat
...
if compTraj
  ...
  % To visualize
  %figure
  %visualizeOptTraj(g, dataTraj, traj, tau2, TrajextraArgs);  
end
end